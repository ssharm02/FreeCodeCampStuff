  ...
  $.getJSON("http://ipinfo.io", function(data) {
    ...
    $.getJSON(UniqueAPIConnectionURL, function(WeatherData) {
      ...
      (function backgroundImageChange() {
        // if weather clear
        if (
          clear.indexOf(currentIconCode) !== -1 &&
          nightTimes.indexOf(hours) !== -1
        ) {
          //Image index Clear
          $("#bg-image, #weather-box").css(
            "background-image",
            "url(" + img[0] + ")"
          );
        } else if (
        ...
        if (
          partlyCloudy.indexOf(currentIconCode) !== -1 &&
          nightTimes.indexOf(hours) !== -1
        ) {
        ...
        // if weather Cloudy
        if (
          cloudy.indexOf(currentIconCode) !== -1 &&
          nightTimes.indexOf(hours) !== -1
        ) {
        ...
        if (
          lightRain.indexOf(currentIconCode) !== -1 &&
          nightTimes.indexOf(hours) !== -1
        ) {
        ...
        if (
          heavyRain.indexOf(currentIconCode) !== -1 &&
          nightTimes.indexOf(hours) !== -1
        ) {
      ...
      (function forecastData() {
        ...
        $(".frc-temp").each(function(i) {
          $(this).data("units", "c");
          $(this).html(
            Math.round(maxtempc[i]) + " - " + Math.round(mintempc[i])
          );
        });
      ...
      (function cityBox() {
        ...
        $("#city-box").prepend(
          '<div class="row"><img class="icon" alt="weather-icon" src="' +
            icon +
            '"></div>'
        );
